...
export default function ContainerDate({ onRemove, index }: ContainerDateProps) {
  ...
  const { data, isLoading, error, refetch } = useGetHeroAwardQuery({ category: category });
  ...
  const militaryRanks = heroAward?.map(award => ({ title: award.title })) || [];
    ...
    <div className={styles.containerDate}>
      <div className={styles.containerDate__boxSelectInput}>
        <Select
          ...
          necessarilySvg
        />
        <Input name={`awards.${index}.year`} title="Год выдачи награды" necessarilySvg />
      </div>
      <TextArea 
